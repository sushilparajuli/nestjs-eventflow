...
export class EventsServiceService implements OnModuleInit {
  ...
  async update(
    ...
    const udpatedData: Record<string, unknown> = { ...updatedEventDto };

    if (updatedEventDto.date) {
      udpatedData.date = new Date(updatedEventDto.date);
    }
    udpatedData.updatedAt = new Date();
    ...
    this.kafkaClient.emit(KAFKA_TOPICS.EVENT_UPDATED, {
      eventId: updated.id,
      changes: Object.keys(updatedEventDto),
      timestamp: new Date().toISOString(),
    });
